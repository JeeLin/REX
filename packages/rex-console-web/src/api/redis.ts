...
export async function scan(sessionId: string, pattern = '*', count = 100): Promise<KeyInfo[]> {
  const res = await fetch(`${API_BASE}/scan?session_id=${sessionId}&pattern=${encodeURIComponent(pattern)}&count=${count}`)
...
export async function getValue(sessionId: string, key: string): Promise<RedisValue> {
  const res = await fetch(`${API_BASE}/key?session_id=${sessionId}&key=${encodeURIComponent(key)}`)
...
export async function getTtl(sessionId: string, key: string): Promise<number> {
  const res = await fetch(`${API_BASE}/ttl?session_id=${sessionId}&key=${encodeURIComponent(key)}`)
...
export async function getInfo(sessionId: string): Promise<RedisInfo> {
  const res = await fetch(`${API_BASE}/info?session_id=${sessionId}`)
