...
| ⬜ | 🔴 | Agent 二进制下载需要授权 | 用户反馈 | 访问 /api/agents/download?os=windows&arch=amd64 时弹出授权提示，Docker 部署下无法正常下载 Agent 二进制 |
| ⬜ | 🔴 | SSH 连接默认用户名未生效 | 用户反馈 | 连接 SSH 资源时提示 username is empty，应默认使用 root 作为 SSH 用户名 |
