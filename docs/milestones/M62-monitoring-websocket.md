...
| 状态 | 优先级 | 标题 | 来源 | 描述 |
|------|--------|------|------|------|
| [x] | 🔴 | M62: SQLite 新建资源路径错误 | 用户反馈 | host 字段硬编码为 localhost 而非用户输入的文件路径 |
| [x] | 🟡 | M62: Agent Token 按钮缺 i18n 和重置 | 用户反馈 | 按钮文字硬编码，缺少重置按钮 |
| [x] | 🟡 | M62: 直连环境不需要 Agent Token | 用户反馈 | 直连模式下不应显示 Agent Token 相关 UI |
| [x] | 🟡 | M62: 新建资源连接方式应由环境决定 | 用户反馈 | 向导中不应让用户选择连接方式，应继承环境设置 |
| [x] | 🟡 | M62: 新建资源向导应合并为单页 | 用户反馈 | 所有步骤（协议选择、名称/颜色、连接详情、确认）应合并为一个页面 |
| [x] | 🟡 | 连接树资源项状态列显示为空 | 用户反馈 | 连接树中资源项有名称但状态列（cs/cm/csc）全部显示为"-"，应该是显示环境/协议/状态等信息 |
| [x] | 🔴 | Agent 二进制下载需要授权 | 用户反馈 | 访问 /api/agents/download?os=windows&arch=amd64 时弹出授权提示，Docker 部署下无法正常下载 Agent 二进制 |
| [x] | 🔴 | SSH 连接默认用户名未生效 | 用户反馈 | 连接 SSH 资源时提示 username is empty，应默认使用 root 作为 SSH 用户名 |
