...
| 状态 | 优先级 | 标题 | 来源 | 描述 |
|------|--------|------|------|------|
| ⬜ | 🟡 | 上下分栏点击后仍是左右分栏 | 用户反馈 | 终端/面板分栏切换：点击「上下分栏」后实际渲染为左右分栏，分栏方向逻辑疑似反了或未生效 |
| ⬜ | 🟡 | 终端日志缺少资源名称 | 用户反馈 | rex_hub::terminal_ws 的 SSH_DISCONNECT 等日志只打印 resource_id（如 23810ae8-...），未显示可读的资源名称，排查时无法直观对应到具体资源 |
| ⬜ | 🟢 | 加一个快捷键指南（右下角） | 用户反馈 | 建议在界面右下角放置快捷键速查指南，方便用户了解可用快捷键 |
