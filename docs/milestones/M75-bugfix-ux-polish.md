...
| 状态 | 优先级 | 标题 | 来源 | 描述 |
|------|--------|------|------|------|
| ⬜ | 🔴 | SSH 终端问题多，需按 bak 版本重写 | 用户反馈 | 当前 SSH 终端存在诸多问题（包括字间距过大等），bak 目录中的版本运行正常。建议以 bak 版本代码为基准重写，当前版本仅做参考 |
...
| ⬜ | 🟡 | SSH 终端不支持多开 | 用户反馈 | SSH 终端需要支持多开多个会话，当前多开功能不正常 |
| ⬜ | 🟡 | 删除资源后侧栏不更新 | 用户反馈 | 删除资源后左侧资源栏不会同步更新，需要刷新页面才能看到变化 |
| ⬜ | 🔴 | isTriggerKey TypeError 崩溃（复发） | 用户反馈 | M74 已修复但再次出现：Cannot read properties of undefined (reading 'toLowerCase')，handleKeyUp 中触发 |
