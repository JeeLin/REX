...
| ⬜ | 🟡 | wt-resource-name 应展示资源名称而非资源ID | 用户反馈 | 工作区终端标题 wt-resource-name 当前显示的是资源ID，应展示资源名称 |
| ⬜ | 🟡 | 工作区复制粘贴功能多处无法使用 | 用户反馈 | 工作区的复制粘贴功能很多都无法使用，需排查剪贴板/右键菜单/快捷键链路 |
| ⬜ | 🔴 | 工作区右上角分栏按钮点击无用 | 用户反馈 | 工作区右上角的分栏按钮点击后无反应，无法正常分栏（与 M79 分屏系统相关） |
...
| ⬜ | 🔴 | IPv6 资源无法连接 | 用户反馈 | 配置为 IPv6 的资源无法连接，需排查地址解析/连接逻辑对 IPv6 的支持 |
| ⬜ | 🟢 | 设置栏更新选项缺少 i18n 覆盖 | 用户反馈 | 设置栏的更新选项好像没有 i18n 覆盖，需补充对应翻译 key |
| ⬜ | 🔴 | agent 配置按页面设置后未生效 | 用户反馈 | 实测：Agent 二进制仅读 REX_HUB_URL/REX_AGENT_TOKEN 等环境变量，无 config.toml/register 子命令，本就默认自动更新。原 bug 实为「部署引导文档用了错误 env 名且虚构了 register/config.toml」，已修正引导文档并移除前端无后端支撑的 auto_update 开关 |
| ⬜ | 🟢 | 环境详情页编辑旁增加删除按键 | 用户反馈 | 环境详情页面右上角编辑旁建议加一个删除按键 |
| ⬜ | 🟢 | 资源编辑增加测试连接按钮 | 用户反馈 | 资源编辑页面缺少测试按钮，建议增加连接测试 |
| ⬜ | 🟢 | 后端日志增加资源名称便于识别 | 用户反馈 | 后端日志最好增加资源名称，不然不好识别是哪条资源的日志 |
| ⬜ | 🟢 | 左右分栏按钮图标显示为田字格 | 用户反馈 | 工作区右上角分栏按钮图标用 ⊞（四宫格），但左右分栏应是左右箭头类图标，图标语义不对 |
