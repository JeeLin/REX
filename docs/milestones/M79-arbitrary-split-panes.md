...
| [x] | 🟡 | 左侧新建资源后已打开环境页未实时刷新 | 用户反馈 | 在左侧新建资源后，之前打开的环境页面没有实时刷新，新资源未展示出来 |
| [x] | 🔴 | IPv6 资源无法连接 | 用户反馈 | 配置为 IPv6 的资源无法连接，需排查地址解析/连接逻辑对 IPv6 的支持 |
| [x] | 🟢 | 设置栏更新选项缺少 i18n 覆盖 | 用户反馈 | 设置栏的更新选项好像没有 i18n 覆盖，需补充对应翻译 key |
| [x] | 🔴 | agent 配置按页面设置后未生效 | 用户反馈 | 实测：Agent 二进制仅读 REX_HUB_URL/REX_AGENT_TOKEN 等环境变量，无 config.toml/register 子命令，本就默认自动更新。原 bug 实为「部署引导文档用了错误 env 名且虚构了 register/config.toml」，已修正引导文档并移除前端无后端支撑的 auto_update 开关 |
| [x] | 🟢 | 环境详情页编辑旁增加删除按键 | 用户反馈 | 环境详情页面右上角编辑旁建议加一个删除按键 |
| [x] | 🟢 | 资源编辑增加测试连接按钮 | 用户反馈 | 资源编辑页面缺少测试按钮，建议增加连接测试 |
| [x] | 🟢 | 后端日志增加资源名称便于识别 | 用户反馈 | 后端日志最好增加资源名称，不然不好识别是哪条资源的日志 |
| [x] | 🟢 | 左右分栏按钮图标显示为田字格 | 用户反馈 | 工作区右上角分栏按钮图标用 ⊞（四宫格），但左右分栏应是左右箭头类图标，图标语义不对 |
