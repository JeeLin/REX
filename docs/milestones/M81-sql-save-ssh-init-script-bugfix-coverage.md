...
| ⬜ | 🟡 | 资源（京东云）日志未覆盖该资源所有相关日志 | 用户反馈 | `resource_name=京东云`，按资源名查看/筛选审计日志时，相关日志未全部覆盖（部分操作未记录 resource_id 或筛选/回显遗漏）。待定位是写入时未带 resource_id，还是前端按资源名过滤不全面。 |
| ⬜ | 🟡 | 重启服务后工作区连接栏出现两个相同资源（旧+新） | 用户反馈 | 重启服务后，点击资源连接跳转工作区时，顶部连接栏（tab 栏）出现一个旧 tab 和一个新 tab，两个同一资源重复。疑为服务重启后 tab 持久化状态与重新打开动作叠加，未去重（按 resource_id 判重）。 |
