...
| **v0.74.0** | DBX 借鉴 P0 实现 + 缺陷池修复（minor，键盘快捷系统 / Quick Open / 右键菜单 / Windows 服务注册） | — | ✅ 已完成（v0.74.0） |
| **v0.74.1** | 缺陷池 bug 批量修复（patch） | — | ✅ 已完成（v0.74.1） |
| **v0.75.0** | DBX P1 — 连接树交互增强 + 数据网格增强（minor，单击/双击激活、列排序/过滤/多选） | — | ✅ 已完成（v0.75.0） |
| **v0.76.0** | DBX P1 — 搜索系统升级 + Tab 管理增强（minor，Quick Open 模糊匹配、Tab 历史/恢复/Pin） | — | ✅ 已完成（v0.76.0） |
| **v0.77.0** | DBX P2 — 文件管理交互 + SSH 终端增强（minor，F2 重命名、重复 SSH 连接修复） | — | ✅ 已完成（v0.77.0） |
| **v0.78.0** | 缺陷池清理 + 文件管理交互完善（minor，拖拽上传、批量选择、SSH vim 卡死修复） | — | ✅ 已完成（v0.78.0） |
| **v0.79.0** | 高级功能 + 质量收尾（minor） | — | ✅ 已完成（v0.79.0） |
| **v0.80.0** | 工作空间增强（minor，布局导出/导入、多窗口、SSH ProxyJump、IPv6 双栈） | — | ✅ 已完成（v0.80.0） |
| **v0.81.0** | SSH Docker 集成（minor，终端工具栏 Docker 按钮：容器列表/启停/日志/进入容器） | — | ✅ 已完成（v0.81.0） |
| **v0.81.1** | 移除 REX_STATIC_DIR，前端 include_dir 嵌入二进制（patch） | — | ✅ 已完成（v0.81.1） |
| **v0.81.2** | CI Docker 构建修复 + README 双语（patch） | — | ✅ 已完成（v0.81.2） |
| **v0.82.0** | Agent 本地 Web 访问（minor，Agent 嵌入前端 + /api/health + 模式自适应 UI） | — | ✅ 已完成（v0.82.0） |
| **v0.83.0** | 缺陷池 Bug 修复 + UX 优化（minor，全局搜索 Cmd+K、拓扑连线贝塞尔曲线） | — | ✅ 已完成（v0.83.0） |
| **v0.84.0** | 数据对比 + MongoDB 支持 + Redis Pub/Sub（minor，查询结果对比、Mongo 连接/查询、Pub/Sub 监控） | — | ✅ 已完成（v0.84.0） |
| **v0.85.0** | Agent API 隧道转发（minor，修复 Agent 内网 API 调用 404） | — | ✅ 已完成（v0.85.0） |
| **v0.86.0** | 多 SQL 方言支持（minor，ClickHouse / SQL Server / MariaDB / Oracle 连接器） | — | ✅ 已完成（v0.86.0） |
| **v0.86.1** | 缺陷池 bug 修复（patch） | — | ✅ 已完成（v0.86.1） |
| **v0.87.0** | Agent/Hub 公共逻辑收敛（minor，SQL 方言整合） | — | ✅ 已完成（v0.87.0） |
| **v0.87.1** | 缺陷池修复（patch） | — | ✅ 已完成（v0.87.1） |
...
### v0.71.3 缺陷池清理 + i18n 补全 ✅ 已完成（v0.71.3）

**核心功能**：修复 v0.71.2 后用户报告的 6 条遗留问题（1 🔴 + 4 🟡 + 1 🟢），并完成全站 i18n 硬编码字符串扫描补全。

**子任务预估**：3-4 个
1. **修复 🔴 SFTP 抽屉接口**：定位 `/api/files/connect` 在 SSH 上下文调用失败原因，修复后端路由或前端调用链
2. **修复 🟡 UX 与登录页问题**：
   - env-card-protocols 添加底部 padding
   - 登录页底部"在宿主机重置密码"提示：移除/改写为文档链接或保留但确保与设计文档 §3.1 一致
   - 首页语言切换按钮：检查 toggleLanguage 实际是否生效（可能与 i18n 初始化覆盖冲突）
3. **🟢 侧栏新建资源按钮**：移除侧栏底部冗余的 new-resource-btn + 提升环境项右侧加号图标的视觉识别度
4. **🟡 全站 i18n 扫描补全**：扫描所有 .vue 文件，识别硬编码中英文字符串，提取为 i18n key 并补全 zh.json/en.json

**依赖**：v0.71.2（基线）
**版本类型**：patch（仅 bug 修复与文案，无新功能）
**版本号**：v0.71.3
**缺陷池 bug**：
- 🔴 /api/files/connect 无法打开 SSH 的 SFTP
- 🟡 首页语言切换按钮无法使用
- 🟡 登录页底部"在宿主机上重置密码"提示无对应功能
...
**子任务预估**：3-4 个
1. **拓扑数据模型与 API**：定义 Env/Resource/Agent/Connection 的拓扑图数据结构，后端提供 `/api/topology` 接口返回图节点与边
2. **拓扑图渲染引擎**：选型（如 vue-flow / d3-force / SVG 手绘），实现节点（环境/资源）、边（Agent 隧道/直连）、状态叠加（在线/离线）、缩放/平移
3. **交互与 UI 集成**：点击节点跳转到资源详情、状态实时刷新、拖拽重排、拓扑视图切换到 List 视图
4. **响应式与移动端适配**：拓扑图在小屏幕下的降级展示（列表模式或可缩放 SVG）

**依赖**：v0.71.3（Bugfix 里程碑完成后继续）
**版本类型**：minor（新功能）
**版本号**：v0.72.0

### v0.73.0：测试覆盖补全 + 质量门禁 ✅ 已完成（v0.73.0）
...
**子任务预估**：3 个
1. **定位问题根因**：分析 Agent SSH 隧道中命令输入失败的原因（可能是终端初始化/PTY 分配/数据转发问题）
2. **修复 SSH 连接**：修复 Agent 模式下 SSH 会话的命令输入和后续连接问题
3. **回归测试 + 质量门禁**：验证直连/Agent 模式 SSH 均可用，全量质量门禁通过

**依赖**：v0.73.0
**版本类型**：patch
**版本号**：v0.73.1
**缺陷池 bug**：Agent SSH 连接后无法输入命令，后续连接全部失败（🔴）（从 docs/BUGS.md 纳入，已在规划时从缺陷池删除）

### v0.74.0：DBX 借鉴 P0 实现 + 缺陷池修复 ✅ 已完成
- **核心功能**：键盘快捷系统（集中注册表+平台感知+Quick Open）+ 右键菜单上下文感知 + 2 个缺陷池 bug 修复
- **子任务预估**：6 个（快捷系统、Quick Open、右键菜单、Windows 服务、连接去重、质量门禁）
- **依赖**：v0.73.4
- **版本类型**：minor
- **版本号**：v0.74.0
|- **缺陷池 bug**：Windows service install 不支持（🟡）、Agent 启动瞬间页面重连导致重复 SSH 连接（🟡）（从 docs/BUGS.md 纳入，已在规划时从缺陷池删除）

### v0.74.1：缺陷池 bug 批量修复 ✅ 已完成
- **核心功能**：修复缺陷池中积累的 11 个 bug（4 个 🔴 + 7 个 🟡）
- **子任务预估**：6 个（SSH/SFTP 并发会话、Agent SSH 连接问题、SQL dialect 自动检测、Docker 二进制下载、设置页 UI 修复、拓扑图布局）
- **依赖**：v0.74.0
- **版本类型**：patch
- **版本号**：v0.74.1
|- **缺陷池 bug**：已连接SSH打开SFTP报错并发会话不支持（🔴）、SSH终端vim打开文件后卡死（🔴）、Agent连接SQL报unrecognized dialect（🔴）、通过Agent连接jump server失败（🔴）、Windows service install 不支持（🟡）、Agent 启动瞬间页面重连导致重复 SSH 连接（🟡）、刷新页面会跳出文件下载（🟡）、Docker启动后仍从GitHub下载agent二进制（🟡）、拓扑图布局不符合预期（🟡）、设置页保存提示样式及按钮位置异常（🟡）、设置中个人资料不会实际保存（🟡）（从 docs/BUGS.md 纳入，已在规划时从缺陷池删除）

### v0.75.0：DBX P1 — 连接树交互增强 + 数据网格增强 ✅ 已完成
- **核心功能**：连接树单击/双击模式、连接状态实时显示、搜索增强（类型前缀+正则）、多选操作、拖拽排序、复制粘贴、收藏/最近；数据网格列头排序、列级过滤、内联编辑预览、多选行、单元格详情、条件编辑器、转置视图
- **子任务预估**：7 个（连接树核心交互、连接树搜索增强、数据网格排序过滤、数据网格编辑预览、数据网格多选详情、条件编辑器、转置视图）
- **依赖**：v0.74.1
- **版本类型**：minor
- **版本号**：v0.75.0

### v0.76.0：DBX P1 — 搜索系统升级 + Tab 管理增强 + 页面级优化 ✅ 已完成
- **核心功能**：Quick Open 增强（模糊匹配+高亮）、侧栏搜索增强（正则+类型前缀）、SQL 全局搜索、历史搜索；Tab 历史导航、关闭恢复、Pin、拖拽排序、关闭确认、上下文菜单；Tab 状态持久化、欢迎页、工具栏可配置、多窗口、批量操作
- **子任务预估**：6 个（搜索系统核心、Tab 管理核心、Tab 上下文菜单、欢迎页、工具栏可配置、多窗口支持）
- **依赖**：v0.75.0
- **版本类型**：minor
- **版本号**：v0.76.0

### v0.77.0：DBX P2 — 文件管理交互 + SSH 终端增强 + AI 助手集成 ✅ 已完成
- **核心功能**：文件拖拽上传/移动、内联重命名、批量选择、文件预览、传输进度；SSH 终端右键菜单增强、终端内搜索、多路复用（Split）、保活检测；AI 助手选中文本→AI、AI 生成 SQL、AI 解释查询、AI 优化建议、上下文感知
- **子任务预估**：6 个（文件管理交互、SSH 终端增强、AI 助手核心、AI 上下文感知、保活检测、质量门禁）
- **依赖**：v0.76.0
- **版本类型**：minor
- **版本号**：v0.77.0
---
...
- **核心功能**：整体重设计「Agent 嵌入前端 ↔ 后端」访问通道，根因已确认——前端 WS 相对同源构造（`WorkspaceTerminal.vue:324-326`），Agent `http_server` 路由表仅 `/api/health` + `/api/*` + 静态 fallback，无 `/ws/*`，`/ws/terminal`、`/ws/sip` 落入 SPA fallback 返回 200 text/html（非 101）握手失败；直连 SSH 失败实为终端通道全断（SQL/Redis/文件走 `/api/*` 正常）。**2026-09-24 用户确认改判 Alt 1（Agent 标准反向代理直打 `REX_HUB_URL`）**，推翻原方案 C（隧道流式帧 + Hub 明文回环自代理，否决理由入档架构文档「方案 C 否决记录」）：Agent 退化为「静态资源 + health 直答 + 标准反代」，`/api/*`（health 除外）经 reqwest 流式反代、`/ws/{*path}` 经 tokio-tungstenite 消息级互转直打 `REX_HUB_URL`（头策略剥 hop-by-hop、Host=Hub、Origin/Referer 改写为 Hub origin 绕 CSRF 校验、补 X-Forwarded-*；只设 connect timeout 禁设 total timeout；TLS 信任复用 `resolve_hub_tls_settings`；scheme 解析统一 `hub_origin()`）；`/ws/agent` 隧道只承担控制面 + 文件传输 Binary 帧（Hub→内网腿不变）；删除 `api_request`/`api_response` 旧机制与 Hub 回环自调（TLS 模式 502 随之自然消失，无需回环 listener）；agent 模式 direct 环境语义=可见可连、路径经 Hub、前端零分支。否决方案 B（前端直连 Hub origin，破坏 agent 前端存在意义）；Agent 嵌入前端页面默认关闭、显式开启（`REX_AGENT_HTTP_PORT` 未设置或 0 不启动 HTTP server，用户决策 2026-09-24）
- **子任务预估**：8 个（S1 设计文档、S1′ 文档改写（C→Alt 1）、S-fix hub_origin 统一与 https→ws 修复、S4 Agent 反代重构与删旧机制、S5 前端 direct 语义、S6 token 脱敏、S7 验证对齐、S8 HTTP server 默认关闭；原 S2 流式帧协议 / S3 回环 listener 已取消）
- **依赖**：v0.88.0
- **版本类型**：minor
- **版本号**：v0.89.0
...
- **触发条件**：Hub 远程部署下实测通话质量/文件吞吐不可接受，或 VPS 带宽成本成为问题
- **前置核实**：agent 模式 SIP 实际路径（v0.89 exp 结论）；若发现正确性问题则先修 bug，本候选顺延
- **状态**：条件触发候选，不锁版本、不占 v0.91 位；`milestone-planner` 规划时按触发条件吸纳，不触发则继续停留候选区。v0.89 里程碑「不做 hairpin 短路优化」条目的复议出口即本条目

## 7. UI/UX 优化路线图（参考 DBX）

> 基于对 DBX（开源数据库管理工具）的深度分析，提炼可借鉴的操作模式与交互逻辑。详见 `docs/PRODUCT.md` §11。

### 待规划里程碑

| 优先级 | 里程碑 | 内容 | 预估工时 |
|--------|--------|------|----------|
| P0 | 键盘快捷系统 | 集中注册表 + 平台感知 + 用户自定义 + Quick Open + 禅模式 | ✅ v0.74.0 已完成 |
| P0 | 右键菜单上下文感知 | 动态菜单项 + 快捷键提示 + 子菜单 + 危险标记 | ✅ v0.74.0 已完成 |
| P1 | 连接树交互增强 | 单击/双击模式 + 搜索增强 + 多选操作 + 拖拽排序 | 📋 v0.75.0 已规划 |
| P1 | 数据网格增强 | 列头排序 + 列级过滤 + 内联编辑预览 + 多选行 | 📋 v0.75.0 已规划 |
| P1 | 搜索系统升级 | Quick Open + 侧栏搜索增强 + SQL 全局搜索 | 📋 v0.76.0 已规划 |
| P1 | Tab 管理增强 | 历史导航 + 关闭恢复 + Pin + 拖拽分屏 + 关闭确认 | 📋 v0.76.0 已规划 |
| P1 | 页面级优化 | Tab 状态持久化 + 欢迎页 + 工具栏可配置 | 📋 v0.76.0 已规划 |
...
### v0.80.0：工作空间增强 ✅ 已完成
- **核心功能**：多窗口支持、工具栏可配置、工作空间导出/导入
- **子任务预估**：4 个（多窗口拆分、工具栏定制、工作空间导入导出、质量收尾）
- **依赖**：v0.79.0
- **版本类型**：minor
- **版本号**：v0.80.0

### v0.81.0：SSH Docker 集成 ✅ 已完成
- **核心功能**：SSH 终端工具栏新增 Docker 按钮，支持容器列表/启停/日志/进入容器等操作
- **子任务预估**：3 个（Docker 按钮 UI + 容器操作面板 + docker exec 进入容器）
- **依赖**：v0.80.0
- **版本类型**：minor
- **版本号**：v0.81.0

### v0.82.0：Agent 本地 Web 访问 ✅ 已完成
- **核心功能**：Agent 嵌入前端 dist，支持本地 HTTP 服务，浏览器直连管理
- **子任务预估**：3 个（Agent HTTP server + 前端嵌入 + 模式自适应 UI）
- **依赖**：v0.81.1
- **版本类型**：minor
- **版本号**：v0.82.0

### v0.83.0：缺陷池 bug 修复 + UX 优化 ✅ 已完成
- **核心功能**：修复缺陷池中的 3 个 bug（Agent 健康状态、全局搜索、拓扑连线优化）
- **子任务预估**：3 个（Agent 健康状态过滤、全局搜索 Cmd+K、拓扑连线曲线化）
- **依赖**：v0.82.0
- **版本类型**：patch
- **版本号**：v0.83.0
...
### v0.87.4：用户反馈缺陷修复 + 文档刷新 ✅ 已完成
- **核心功能**：修复 8 条用户反馈 bug（登录页主题切换按钮、SSH 终端 Docker 按钮移除、设置页对齐、设置页导览吸顶、关于页服务器信息、Agent 健康状态过滤直连环境、审计日志分页、Agent 更新残留 .bak/.old 回归）+ docs/ 文档刷新 + .env 内容整理
- **子任务**：10 个（8 bug + 文档刷新 + .env 整理）
- **依赖**：v0.87.3
- **版本类型**：patch
- **版本号**：v0.87.4
- **来源**：用户反馈（8 🟡 直接纳入，未走缺陷池）
