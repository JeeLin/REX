...
| 优先级 | 里程碑 | 内容 | 预估工时 |
|--------|--------|------|----------|
| P0 | 键盘快捷系统 | 集中注册表 + 平台感知 + 用户自定义 + Quick Open + 禅模式 | 2-3 天 |
| P0 | 右键菜单上下文感知 | 动态菜单项 + 快捷键提示 + 子菜单 + 危险标记 | 2-3 天 |
| P1 | 连接树交互增强 | 单击/双击模式 + 搜索增强 + 多选操作 + 拖拽排序 | 3-4 天 |
| P1 | 数据网格增强 | 列头排序 + 列级过滤 + 内联编辑预览 + 多选行 | 3-4 天 |
| P1 | 搜索系统升级 | Quick Open + 侧栏搜索增强 + SQL 全局搜索 | 2 天 |
| P1 | Tab 管理增强 | 历史导航 + 关闭恢复 + Pin + 拖拽分屏 + 关闭确认 | 2 天 |
| P1 | 页面级优化 | Tab 状态持久化 + 欢迎页 + 工具栏可配置 | 2-3 天 |
| P2 | 文件管理交互 | 拖拽上传/移动 + 内联重命名 + 文件预览 | 2-3 天 |
| P2 | SSH 终端增强 | 右键菜单增强 + 终端内搜索 + 多路复用 + 保活检测 | 2-3 天 |
| P2 | AI 助手集成 | 选中文本→AI + AI 生成 SQL + 上下文感知 | 3-4 天 |
