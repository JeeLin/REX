...
- **Agent 健康状态**：过滤直连环境（v0.83.0 同类问题回归）；空态文案改「暂无经 Agent 接入的环境」并补双语言键
- **审计日志**：补翻页控件（页大小 20/50/100/200），后端 limit/offset 端到端生效；列表改为页内滚动、分页条常显
- **快捷键入口**：右下角悬浮按钮改为应用顶栏普通按钮（退出登录左侧），全页面可用，不再遮挡内容
...
### Added
- **全局搜索**：顶部导航栏支持 Cmd+K 快捷键打开全局搜索面板，可快速跳转到资源、环境、设置项

### Fixed
- **Agent 健康状态**：Agent 模式下仪表盘仅显示通过 Agent 隧道连接的环境，不再显示直连环境
- **拓扑连线布局**：拓扑视图连线改为贝塞尔曲线，避免连线重叠交叉
...
### Added
- **工作空间导出/导入**：保存和恢复工作空间布局配置（JSON 格式）
- **多窗口支持**：右键菜单「在新窗口中打开」资源
- **工具栏可配置**：自定义显示/隐藏工具栏按钮（localStorage 持久化）

### Fixed
- **分栏重复调用**：修复 splitVertical() 中 splitPane() 重复调用导致分栏两次的 bug
...
### Added
- **文件拖拽上传**：拖拽外部文件到面板自动上传（支持进度追踪）
- **文件批量选择**：Shift/Ctrl 多选 + 批量下载/删除操作栏

### Fixed
- **SSH vim 卡死修复**：添加 60s stall timeout + Semaphore(16) 写入限制 + 全面日志
- **SQL dialect 检测改善**：错误收集 + 诊断日志 + 可操作建议
...
### Added
- **连接树激活模式**：支持配置单击/双击激活资源（localStorage 持久化）
- **连接树状态显示**：每个资源节点显示 agent 在线/离线/连接中状态指示灯
- **连接树搜索增强**：支持类型前缀过滤（sql:/ssh:/redis:）和正则模式
- **连接树多选操作**：Ctrl/Shift 多选资源节点，批量连接/断开
- **数据网格列头排序**：点击排序（升序→降序→取消）+ 多列排序
- **数据网格列级过滤**：列头筛选图标，支持 =, !=, LIKE, >, < 运算符
- **数据网格多选行**：Shift/Ctrl 多选行，批量导出选中行
- **数据网格单元格详情**：双击单元格显示完整内容（JSON 格式化）

### Changed
- 缺陷池复杂 bug（SSH vim卡死/SQL dialect/ProxyJump/Windows service/IPv6）推迟至 v0.75.1
